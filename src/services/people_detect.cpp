...
void SimplePeopleDetector::saveEngine(const std::string& filePath, IHostMemory* serializedModel) {
...
bool SimplePeopleDetector::loadEngine(const std::string& filePath){
...
SimplePeopleDetector::SimplePeopleDetector(const std::string& onnxPath)
    : enginePath(onnxPath), engine(nullptr), context(nullptr) {

    std::string saveFileName = "people_detect.engine";
...
bool SimplePeopleDetector::isInitialized() const {
    return runtime && engine && context && stream && network_input_h_ > 0 && network_input_w_ > 0;
}

size_t SimplePeopleDetector::calculateSizeFromDims(const Dims& dims) {
    size_t size = 1;
...
void SimplePeopleDetector::preprocess(const Mat& frame, std::vector<float>& cpu_input_buffer, int input_w, int input_h) {
    Mat resized, rgb;
...
std::vector<cv::Rect> SimplePeopleDetector::detect(const Mat& frame) {
...
int SimplePeopleDetector::getInputWidth() const { return network_input_w_; }
int SimplePeopleDetector::getInputHeight() const { return network_input_h_; }
