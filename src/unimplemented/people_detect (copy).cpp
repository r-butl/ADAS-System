// Full updated file with dynamic tensor name detection
...
void drawBoxes(Mat& frame, float* output_data, int num_detections, float conf_threshold = 0.5) {
    const int props = 6;
    for (int i = 0; i < num_detections; ++i) {
        ...
        putText(frame, "Object " + to_string(int(confidence * 100)) + "%", Point(x1, y1 - 5), FONT_HERSHEY_SIMPLEX, 0.6, Scalar(0, 255, 0), 1);
    }
}
...
public:
    SimplePeopleDetector(const string& path)
        ...
        for (int i = 0; i < engine_->getNbIOTensors(); ++i) {
            ...
            if (engine_->getTensorIOMode(name) == TensorIOMode::kINPUT && !input_tensor_name_) input_tensor_name_ = name;
            else if (engine_->getTensorIOMode(name) == TensorIOMode::kOUTPUT && !output_tensor_name_) output_tensor_name_ = name;
        }
